...
from mytig.serializers import CoquillageSerializer, CrustaceSerializer, PoissonSerializer, ProduitEnPromotionSerializer
    ...
    def handle(self, *args, **options):
        ...
        for product in jsondata:
            if product['category'] == 0:
                ...
                serializer = CoquillageSerializer(
                    data={'tigID': str(product['id'])})
                if serializer.is_valid():
                    serializer.save()
                    self.stdout.write(self.style.SUCCESS(
